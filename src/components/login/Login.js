...
const Login = () => {
   ...
   *  
   */
  const { handleChange, values, errors, handleSubmit } = useForm(validate, setLoading);
  ...
  if(user) {
     ...
     */}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <br></br>
          <p className="text-danger login-error text-right mr-5"> {errors.phone}</p>
          <input
            type="password"
            name="password"
            className="text-right login-input"
          ...
          <button disabled={loading ? true : false}
            className='login-button' id="login-button" type="submit">
            {loading ? <BeatLoader color="white" loading={loading} size={8} css={override} /> : 'ورود'}
          </button>
          <br></br>
        </form>
      </div>
